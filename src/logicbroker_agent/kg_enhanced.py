"""Enhanced Knowledge Graph Retriever with node embeddings and community summaries.

Two key improvements over the keyword-based KG retriever:
1. Node embeddings — cosine similarity replaces keyword matching for seed node discovery
2. Community detection + LLM summaries — Louvain clustering with per-community summaries
   that capture multi-hop relationships in a single retrievable chunk
"""
...
class EnhancedKnowledgeGraphRetriever:
    """KG retriever using embedding-based node lookup and community summaries."""
    ...
    def __init__(
        ...
        self._node_embeddings = self._load_or_build_node_embeddings(kg_hash)
        self._communities = self._detect_communities()
        self._community_summaries = self._load_or_build_community_summaries(kg_hash)
        self._community_embeddings = self._load_or_build_community_embeddings(kg_hash)

        logger.info(
            f"Enhanced KG loaded: {len(self._nodes)} nodes, "
    ...
    def query(self, query: str, max_results: int = 30, max_hops: int = 3) -> list[str]:
        """Query using hybrid node matching (keyword + embedding RRF) + community summaries."""
        if not self._nodes:
            return []
    ...
    def _keyword_rank_nodes(self, query: str) -> list[str]:
        """Rank nodes by keyword overlap (same logic as baseline KG retriever)."""
    ...
    def _score_edges_by_embedding(
        ...
        # Serialize edges to text
        edge_texts = [
            f"{subj} {pred} {obj} {ctx}" for subj, pred, obj, ctx in edges
        ]

        # Batch embed
        edge_embeddings = self._model.encode(edge_texts)
        query_norm = query_embedding / (np.linalg.norm(query_embedding) + 1e-8)
    ...
    def _load_or_build_node_embeddings(self, kg_hash: str) -> np.ndarray:
        """Load cached node embeddings or build them."""
        cache_path = self._cache_dir / f"node_embeddings_{kg_hash}.npy"
        if cache_path.exists():
            logger.info(f"Loading cached node embeddings from {cache_path}")
            return np.load(cache_path)

        logger.info(f"Building node embeddings for {len(self._nodes)} nodes...")
        embeddings = self._model.encode(self._nodes, show_progress_bar=True)
    ...
    def _load_or_build_community_summaries(self, kg_hash: str) -> list[str]:
        """Load cached community summaries or build them."""
        ...
        for i, community in enumerate(self._communities):
            ...
            if (i + 1) % 10 == 0:
                logger.info(f"  Summarized {i+1}/{len(self._communities)} communities")

        cache_path.write_text(json.dumps(summaries, indent=2))
        return summaries

    def _load_or_build_community_embeddings(self, kg_hash: str) -> np.ndarray:
        """Load cached community summary embeddings or build them."""
        cache_path = self._cache_dir / f"community_embeddings_{kg_hash}.npy"
        if cache_path.exists():
            return np.load(cache_path)
        ...
        embeddings = self._model.encode(self._community_summaries, show_progress_bar=True)
        np.save(cache_path, embeddings)
        return embeddings

    def _summarize_community(self, community: set) -> str:
        """Generate a text summary of a community's nodes and internal edges.

        Uses a deterministic template approach (no LLM call) for speed and cost.
        Can be upgraded to LLM summaries later if needed.
        """
        ...
        # Build summary from edges (more informative than just listing nodes)
        if internal_edges:
    ...
    def build_llm_community_summaries(self) -> list[str]:
        """Build LLM-generated community summaries (call once, results are cached).

        This replaces the template summaries with richer LLM-generated ones.
        ...
        # Rebuild community embeddings
        self._community_embeddings = self._model.encode(summaries, show_progress_bar=True)
        emb_cache = self._cache_dir / f"community_embeddings_{kg_hash}.npy"
